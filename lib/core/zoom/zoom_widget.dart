...
import '../../screens/wallpaper_detail/wallpaper_detail.dart';
import '../constants/color_constants.dart';
import 'zoom_fullsreen.dart';
...
class Zoom extends StatefulWidget {
  ...
  /// provide custom hero animation tag and make sure every [Zoom] in your subtree uses a different tag. otherwise the animation doesnt work
  ...
  @override
  State<Zoom> createState() => _WidgetZoomState();
}

class _WidgetZoomState extends State<Zoom> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    //   var provider = Provider.of<SetImageProvider>(context, listen: false);
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        log('widget.zoomWidget is ${widget.zoomWidget}');
  ...
  emptyImage() {
    return Image(
        image: FileImage(File("/data/user/0/com.kavach.security.guard/cache/"),
            scale: 1.0));
  }
    ...
    await _rootNavigator.push(
      PageRouteBuilder(
        ...
        pageBuilder: (context, animation1, animation2) => FadeTransition(
          opacity: animation1,
          child: UltraPremiumWallpaperDetail(
            item: widget.item,
          ),
        ),
